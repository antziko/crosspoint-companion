...
2. ✅ **Unit tests**: `pio run -t unit-tests` (host gtest — no hardware needed; run when touching parsers, dictionary, hyphenation, rendering logic). Note that four suites are shell scripts absent from the CMake project — `test/dict-html-renderer/`, `test/dict-layout/`, `test/dict-word-select-nav/`, `test/lookup-chain/` — so ctest can report green while they are not even compiled. Loop their `run.sh` too.
...
- `book.bin`: **Version 10** — `BOOK_CACHE_VERSION` in `lib/Epub/Epub/BookMetadataCache.cpp`
- `section.bin`: **Version 47** — `SECTION_FILE_VERSION` in `lib/Epub/Epub/Section.cpp`
