...
void BaseTheme::drawHintLabel(GfxRenderer& renderer, const int fontId, const char* label, const int x,
  ...
  // Spaced by the glyph height, not getLineHeight() — that returns the font's full
...
int BaseTheme::getButtonHintSlotCenterX(GfxRenderer& renderer, int slot, const char* /*label*/) const {
  const GfxRenderer::Orientation orig_orientation = renderer.getOrientation();
  renderer.setOrientation(GfxRenderer::Orientation::Portrait);
...
void BaseTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle) const {
  ...
  // rewrite would have silently dropped it. Bottom-aligned to the battery glyph
  // (which batteryIndicator centres in the strip) so the two read as one status
  // cluster, matching the pre-FUI placement. Only drawn for a right-side battery:
  // no feat theme sets headerBatterySide=1, and the bars would collide with the
  // title if placed on the left.
  if (!batteryLeft) {
    const int16_t batteryGlyphBottom =
        static_cast<int16_t>(band.y + (batteryH + static_cast<int16_t>(metrics.batteryHeight)) / 2);
    drawWifiBars(renderer, batteryX - wifiBatterySpacing, batteryGlyphBottom);
  }
