...
bool OpdsServerStore::fromJson(JsonVariantConst doc) {
  ...
  for (JsonObjectConst obj : arr) {
    ...
    // Missing in files written before these fields existed: extraQuery -> "" (no-op
    // append); sortAlphabetical -> true so old servers keep the former global A-Z default.
    server.extraQuery = obj["extra_query"] | "";
