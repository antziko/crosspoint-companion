...
//   [29-32] lastSyncReadingSeconds    uint32_t LE  (total at last successful sync; 0 = never)
//   [33-36] lastSyncPromptSkipSeconds uint32_t LE  (total at last "Skip" of a sync prompt; 0 = never)
//   [37-40] lastInlineReviewSeconds   uint32_t LE  (total at last inline flashcard review; 0 = never)
//
// v6 (37 bytes) is v7's leading layout exactly; parse() upgrades it by zeroing
// lastInlineReviewSeconds. v5 (33 bytes) is v6's leading layout; parse() upgrades it by
// zeroing lastSyncPromptSkipSeconds too. v4 (29 bytes) is v5's leading layout; parse()
// upgrades it by zeroing lastSyncReadingSeconds too. v3 (19 bytes) is v4's leading layout;
// parse() upgrades it by zeroing the remote-sync fields too. All lossless. The previous hard
// version-reject would have silently wiped every book's accumulated reading time on
// firmware upgrade. v1/v2 remain rejected (see v3's history in git).
...
constexpr int STATS_FILE_SIZE_V4 = 29;
constexpr int STATS_FILE_SIZE_V5 = 33;
constexpr int STATS_FILE_SIZE_V6 = 37;
constexpr int STATS_FILE_SIZE = 41;
...
bool BookReadingStats::parse(const uint8_t* data, size_t len, BookReadingStats& out) {
  ...
  const bool v7 = (version == STATS_FILE_VERSION && len == static_cast<size_t>(STATS_FILE_SIZE));
  const bool v6 = (version == STATS_FILE_VERSION_V6 && len == static_cast<size_t>(STATS_FILE_SIZE_V6));
  const bool v5 = (version == STATS_FILE_VERSION_V5 && len == static_cast<size_t>(STATS_FILE_SIZE_V5));
  const bool v4 = (version == STATS_FILE_VERSION_V4 && len == static_cast<size_t>(STATS_FILE_SIZE_V4));
  ...
  // lastSyncReadingSeconds added in v5; older files zero it (first sync resets it).
  out.lastSyncReadingSeconds = (v7 || v6 || v5) ? readLe32(data, 29) : 0;
  // lastSyncPromptSkipSeconds added in v6; older files zero it (no deferral pending).
  out.lastSyncPromptSkipSeconds = (v7 || v6) ? readLe32(data, 33) : 0;
  // lastInlineReviewSeconds added in v7; older files zero it, so the first inline review
  // fires once the interval's worth of reading has accrued from the book's odometer.
  out.lastInlineReviewSeconds = v7 ? readLe32(data, 37) : 0;
  return true;
}
...
void BookReadingStats::save(const std::string& cachePath) const {
  ...
  writeLe32(data, 29, lastSyncReadingSeconds);
  writeLe32(data, 33, lastSyncPromptSkipSeconds);
  writeLe32(data, 37, lastInlineReviewSeconds);
