...
//
// v5 (33 bytes) is v6's leading layout exactly; parse() upgrades it by zeroing
// lastSyncPromptSkipSeconds. v4 (29 bytes) is v5's leading layout; parse() upgrades it
// by zeroing lastSyncReadingSeconds too. v3 (19 bytes) is v4's leading layout; parse()
// upgrades it by zeroing the remote-sync fields too. All lossless. The previous hard
...
constexpr int STATS_FILE_SIZE = 37;
...
bool BookReadingStats::parse(const uint8_t* data, size_t len, BookReadingStats& out) {
  ...
  const bool v6 = (version == STATS_FILE_VERSION && len == static_cast<size_t>(STATS_FILE_SIZE));
  ...
  out.lastSyncReadingSeconds = (v6 || v5) ? readLe32(data, 29) : 0;
  // lastSyncPromptSkipSeconds added in v6; older files zero it (no deferral pending).
  out.lastSyncPromptSkipSeconds = v6 ? readLe32(data, 33) : 0;
  return true;
}
