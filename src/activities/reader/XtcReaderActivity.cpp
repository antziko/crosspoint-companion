...
void XtcReaderActivity::openChapterSelection() {
  if (xtc && xtc->hasChapters() && !xtc->getChapters().empty()) {
    startActivityForResultNoThrow<XtcReaderChapterSelectionActivity>(
...
void XtcReaderActivity::loop() {
  ...
  [[maybe_unused]] auto [prevTriggered, nextTriggered, fromTilt, fromSide] = ReaderUtils::detectPageTurn(mappedInput);
...
void XtcReaderActivity::renderPage() {
  ...
  if (!pageBuffer) {
    ...
    renderer.clearScreen();
    renderer.drawCenteredText(UI_10_FONT_ID, 300, tr(STR_MEMORY_ERROR), true, EpdFontFamily::BOLD);
    renderer.displayBuffer();
    return;
  }
  ...
  if (bytesRead == 0) {
    ...
    free(pageBuffer);
    renderer.clearScreen();
    renderer.drawCenteredText(UI_10_FONT_ID, 300, tr(STR_PAGE_LOAD_ERROR), true, EpdFontFamily::BOLD);
    renderer.displayBuffer();
    return;
  }
