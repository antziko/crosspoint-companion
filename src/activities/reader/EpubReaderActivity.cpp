...
#include "../settings/TextSettingsActivity.h"
...
void EpubReaderActivity::onExit() {
  ...
  // Free the toolbar overlay's page snapshot (up to 48 KB of BW chunks) if one
  // is still held. Upstream does this in a destructor; this fork frees in
  // onExit() like every other activity resource.
  discardOverlayPage();
...
void EpubReaderActivity::loop() {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Confirm) ||
    ...
    } else if (usesToolbarMenu() && section) {
      // Toolbar style: the page is on screen and in the framebuffer, so paint
      // the toolbar over it (one refresh) instead of pushing a full-screen menu.
      pendingManualTurn = 0;
...
void EpubReaderActivity::openReaderMenu() {
  ...
  if (usesToolbarMenu()) {
    ...
    if (!toolbarUi) {
      LOG_ERR("EPUB", "OOM: ReaderToolbarUi");
      overlay = Overlay::None;
      return;
    }
    toolbarUi->begin();
...
void EpubReaderActivity::openOverlay(Overlay target) {
  ...
  // abort()s rather than returning null (AGENTS.md).
  if (!toolbarUi) toolbarUi = makeUniqueNoThrow<ReaderToolbarUi>(renderer);
  if (!toolbarUi) {
    LOG_ERR("EPUB", "OOM: ReaderToolbarUi");
    overlay = Overlay::None;
...
void EpubReaderActivity::buildMoreActions() {
  ...
  // feat's builder returns the vector and gates on hasDictionary (see its
  // LOCAL(feat) note) rather than upstream's out-param + hasBookmarks.
  moreItems = EpubReaderMenuActivity::buildMenuItems(!currentPageFootnotes.empty(),
                                                     Dictionary::exists(epub->getCachePath().c_str()));
  // READER_OPTIONS is this fork's equivalent of upstream's TEXT_SETTINGS row:
  // both are what the Text tool already covers.
  moreItems.erase(std::remove_if(moreItems.begin(), moreItems.end(),
                                 [](const auto& item) {
                                   return item.action == MA::SELECT_CHAPTER || item.action == MA::READER_OPTIONS;
                                 }),
                  moreItems.end());
}
...
std::string EpubReaderActivity::moreRowValue(int row) const {
  ...
  switch (moreItems[row].action) {
    ...
    // No NIGHT_MODE row here: this fork's reader menu has none (it lives on the
    // frontlight panel's tile grid instead).
    case MA::FRONTLIGHT:
