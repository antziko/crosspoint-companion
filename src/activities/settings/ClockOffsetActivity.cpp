...
void ClockOffsetActivity::onEnter() {
  Activity::onEnter();
  loadFromSettings();
  // Start on the sign field so users immediately see they can toggle to negative offsets.
...
void ClockOffsetActivity::loop() {
  ...
  if (mappedInput.wasPressed(MappedInputManager::Button::Confirm)) {
    // Last field → confirm and exit (onExit saves). Otherwise advance to the next field.
    if (activeField == FIELD_COUNT - 1) {
      finish();
      return;
    }
    activeField = static_cast<Field>(activeField + 1);
    requestUpdate();
    return;
  }

  // For value editing, "Up" intuitively means "more" and "Down" means "less", which is the
  // opposite of ButtonNavigator's list-style Next/Previous semantics (Down = next item).
  // Map onPreviousRelease (Up/Left) to +1 and onNextRelease (Down/Right) to -1.
...
void ClockOffsetActivity::render(RenderLock&&) {
  ...
  //   ":"
  //   "{mm}"        -> minutes
  // Use BOLD width: the displayed string is rendered with EpdFontFamily::BOLD, and bold
  // glyph advances differ from regular ones — measuring against REGULAR mis-positions
  // the caret a few pixels to the left of each field.
  ...
  // Caret drawn as a short bar below the active field. drawText() positions the glyph top
  // at y=centreY and the text extends downward by getTextHeight; place the caret 4 px under
  // the bottom of the glyphs so it can never overlap a descender or look like a dangling dash.
  const int caretY = centreY + renderer.getTextHeight(UI_12_FONT_ID) + 4;
  ...
  const auto* confirmLabel = (activeField == FIELD_COUNT - 1) ? tr(STR_OK_BUTTON) : tr(STR_NEXT_FIELD);
