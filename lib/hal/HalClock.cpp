...
void HalClock::begin() {
  if (!gpio.deviceIsX3()) {
    // No DS3231 on this board. Leave _hasRtc false; RAM mode is opt-in via settings.
    _hasRtc = false;
    return;
  }
...
bool HalClock::getTime(uint8_t& hour, uint8_t& minute) const {
  if (!_hasTime) return false;
  if (_hasRtc) return getTimeFromRtc(hour, minute);
  getTimeFromRam(hour, minute);
  return true;
}

bool HalClock::getTimeFromRtc(uint8_t& hour, uint8_t& minute) const {
...
void HalClock::getTimeFromRam(uint8_t& hour, uint8_t& minute) const {
  // Unsigned subtraction handles the 49.7-day millis() rollover correctly: as long as
  // both operands are unsigned long, the difference is the elapsed millis modulo 2^32.
  const unsigned long elapsedMs = millis() - _ramSyncedMillis;
  const unsigned long elapsedSec = elapsedMs / 1000;

  unsigned long totalSec = static_cast<unsigned long>(_ramSyncedHour) * 3600UL +
                           static_cast<unsigned long>(_ramSyncedMinute) * 60UL +
                           static_cast<unsigned long>(_ramSyncedSecond) + elapsedSec;
  totalSec %= 86400UL;  // wrap to 24h

  hour = static_cast<uint8_t>(totalSec / 3600UL);
  minute = static_cast<uint8_t>((totalSec / 60UL) % 60UL);
}
  ...
  for (int i = 0; i < maxAttempts; i++) {
    if (sntp_get_sync_status() == SNTP_SYNC_STATUS_COMPLETED) {
      ...
      if (_hasRtc) {
        ...
        // Seed the RAM anchor. millis() captured here is the reference for advancement.
        _ramSyncedHour = static_cast<uint8_t>(timeinfo.tm_hour);
        _ramSyncedMinute = static_cast<uint8_t>(timeinfo.tm_min);
        _ramSyncedSecond = static_cast<uint8_t>(timeinfo.tm_sec);
        _ramSyncedMillis = millis();
        _hasTime = true;
      }
      _syncedThisSession = true;
      LOG_INF("CLK", "Clock set to %02d:%02d:%02d UTC (%s)", timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec,
              _hasRtc ? "RTC" : "RAM");
      return true;
    }
    delay(100);
