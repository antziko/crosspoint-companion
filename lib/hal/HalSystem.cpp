...
void IRAM_ATTR __wrap_panic_print_backtrace(const void* frame, int core) {
  ...
  // A hardware exception never reaches __wrap_panic_abort, so on both
  // architectures this dump is the only diagnostic a crash leaves on-device --
  // which is why the X4 Pro (Xtensa S3) needs it too, not just the C3.
#if __riscv
  ...
  // A wild stack pointer would fault this loop inside the panic handler, losing
  // the report entirely; hand off to the real handler instead.
  ...
  int depth = 0;
  for (int x = 0; x < static_cast<int>(captureBytes); x += per_line * sizeof(uint32_t)) {
...
std::string getPanicInfo(bool full) {
  if (!full) {
    ...
    // A lockup or hardware watchdog resets without running any panic hook, so the reason and
    // stack both come back empty; the reset cause is then the only way to tell those apart
    // from a true panic.
