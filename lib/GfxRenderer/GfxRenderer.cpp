...
void GfxRenderer::prewarmFallbackText(const int fontId, const TextGetter getter, const void* ctx,
  ...
  // Resolve the fallback id from the first string that actually redirects; a screen with no
  // CJK strings resolves nothing and this whole call is a no-op.
  ...
  // Append one virtual index for U+2026: truncation measures every long row as "label...",
  // so an ellipsis missing from the batch forces a union rebuild on the first repaint.
  struct WrapCtx {
    TextGetter getter;
    const void* ctx;
    uint32_t count;
...
void GfxRenderer::ensureSdGlyphsResident(const int fontId, const char* text, const EpdFontFamily::Style style,
  ...
  // loadKernLig=false: redirected fallback strings (CJK titles, filenames) have no useful
  // kern pairs, and the ~3KB class tables plus the per-rebuild mini matrix cost heap and SD
  // time exactly where these strings live -- heap-tight UI screens, where that size class is
  // also what strands a TLS record buffer. The reader's PrewarmScope path keeps kern; a
  // kern-wanting request that subset-hits a kern-free mini tops the matrix up in prewarmStyle
  // without re-reading any glyphs.
  const uint8_t styleMask = static_cast<uint8_t>(1u << (static_cast<uint8_t>(style) & 0x03));
        ...
        for (int glyphX = 0; glyphX < width; glyphX++, pixelPosition++) {
          ...
          if (renderMode == GfxRenderer::BW && bmpVal < 3) {
            ...
            renderer.drawPixel(screenX, screenY, pixelState);
                     ...
                     renderer.textAntiAlias()) {
            // Light gray (also mark the MSB if it's going to be a dark gray too)
            // Dedicated X3 gray LUTs now provide proper 4-level gray on both devices
            // We have to flag pixels in reverse for the gray buffers, as 0 leave alone, 1 update
            // Skipped in "Sharp" mode (textAntiAlias off): edges stay solid black from the BW pass.
            renderer.drawPixel(screenX, screenY, false);
          } else if (renderMode == GfxRenderer::GRAYSCALE_LSB && bmpVal == 1 && renderer.textAntiAlias()) {
            // Dark gray
            renderer.drawPixel(screenX, screenY, false);
          }
        }
...
void GfxRenderer::drawText(const int fontId, const int x, const int y, const char* text, const bool black,
  ...
  // Baseline from the resolved font; but when the string was redirected to the fallback, the
  // caller positioned this line using the REQUESTED font's metrics (row bands, icon
  // centering), so centre the fallback's line box inside the requested font's line box rather
  // than letting a taller or shorter fallback hang below or float above the row's centre.
