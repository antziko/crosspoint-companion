...
class GfxRenderer {
 ...
 private:
  ...
  // before a per-glyph measure/draw loop runs. Call only when
  // resolveTextFontId redirected the string: UI screens (home, file browser,
  // recents) draw those CJK strings without the reader's PrewarmScope and
  // without an advance table, so every glyph would otherwise fault through
  // SdCardFont::onGlyphMiss -- one .cpfont open + seek + read per glyph, per
  // redraw (#2725). One prewarm per string costs a single file open;
  // re-measuring or re-drawing resident glyphs is a RAM-only subset check.
 ...
 public:
  ...
  // Batch-prewarm CJK fallback glyphs for a screenful of static strings in ONE SD pass. List
  // screens redraw every visible row on each repaint; without an up-front batch each row's
  // draw prewarms per-string, and under heap pressure (union merge declined) each string
  // evicts the previous one -- SD reads on every repaint, forever. Call once when the
  // screen's strings are known (data load); later measures and draws become RAM-only subset
  // hits. No-op when nothing routes to an SD fallback.
  // The getter form fetches strings one at a time and is allocation-free: callers must NOT
  // build a concatenated std::string, whose bare-new growth abort()s on the heap-tight
  // screens this exists for. A null getter result skips that index.
