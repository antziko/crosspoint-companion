...
class SdCardFont {
 ...
 private:
  ...
  struct PerStyle {
    ...
    // max, so page turns stop touching the allocator. Data: the next prewarm
    // subset-checks against the resident tables (see prewarmStyle), so the idle
    // prewarm of page N+1 serves the actual page turn with zero SD reads. Retention
    ...
    // remains the full teardown (zeroes capacities) for style eviction / font unload.
  ...
  // Per-render mini-data reset (PrewarmScope -> clearCache). Unlike freeStyleMiniData
  // this KEEPS the buffers and loaded data (keep-if-fits reuse + idle-prewarm carry),
  // freeing only under a heap floor or sustained bitmap-arena underuse.
