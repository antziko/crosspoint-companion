...
void ChapterHtmlSlimParser::flushPartWordBuffer() {
  // A table cell can flush with no block open; drop the pending part-word rather
  // than carry it into the next cell. Ordered before the soft flush, which is
  // itself a no-op without a block, so nothing is lost by guarding first.
  if (!currentTextBlock) {
...
void ChapterHtmlSlimParser::addLineToPage(std::shared_ptr<TextBlock> line, const uint32_t visibleOffset) {
  ...
  // Allocated BEFORE the link spans are drained: takeLinkSpans() is destructive, and on the
  // OOM path this returns without placing the line, so harvesting first would register tap
  // targets for a line that never made it onto the page.
