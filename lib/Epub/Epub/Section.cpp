...
// v42: Word-gap suppression at a CJK break opportunity now fires only for tokens glued in the
//      source (attachToPrevious); real whitespace between two words renders as a space again,
...
// v50: Simple HTML table rows are laid out as positioned columns instead of
...
void Section::recordBuildFailure(const BuildFailure::Reason reason, const uint32_t floor, const uint32_t htmlSize,
                                 const uint8_t streamSub) {
  // Capture free heap BEFORE any cleanup (Storage.remove / cssParser->clear / file.close
  // recover heap, so sampling after them hides the real failure-point number).
  buildFailure_.reason = reason;
  buildFailure_.failHeap = esp_get_free_heap_size();
  buildFailure_.floor = floor;
  buildFailure_.htmlSize = htmlSize;
...
bool Section::startBuild(const int fontId, const float lineCompression, const bool extraParagraphSpacing,
  ...
  // Inflated (uncompressed) HTML size + last stream sub-reason, hoisted to function scope so
  // the heap-floor guard and the Phase-A failure records below can see them on every path.
  uint32_t inflatedHtmlSize = 0;
  uint8_t streamReason = 0;  // last attempt's ZipFile::StreamResult (Reason::Stream sub-code)
  ...
  if (reusedHtml) {
    ...
    HalFile hf;
    if (Storage.openFileForRead("SCT", htmlPath, hf)) inflatedHtmlSize = hf.size();
  } else {
    ...
    if (!streamed) {
      recordBuildFailure(BuildFailure::Reason::Stream, 0, 0, streamReason);
      LOG_ERR("SCT", "Failed to stream item contents to temp file after retries");
      return false;
    }
    inflatedHtmlSize = fileSize;
  ...
  if (!Storage.openFileForWrite("SCT", binTmpPath(), file)) {
    recordBuildFailure(BuildFailure::Reason::OpenWrite, 0, inflatedHtmlSize);
...
bool Section::commitBuildFile(const uint8_t version, const uint32_t bytesConsumed, const uint32_t totalBytes) {
  ...
  for (const auto& entry : build_->lut) {
    if (entry.fileOffset == 0) {
      recordBuildFailure(BuildFailure::Reason::Lut, 0, buildHtmlSize_);
