...
class Page {
 public:
  ...
  // placeholder pre-pass so the reader shows text immediately, then decodes images).
  bool hasImagesNeedingDecode(const GfxRenderer& renderer) const {
    return std::any_of(elements.begin(), elements.end(), [&renderer](const std::shared_ptr<PageElement>& element) {
      return element->getTag() == TAG_PageImage &&
             static_cast<const PageImage&>(*element).getImageBlock().needsDecode(renderer);
    });
  }
