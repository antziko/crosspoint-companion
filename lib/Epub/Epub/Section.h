...
class Section {
 ...
 public:
  ...
  // Phase-A diagnostics: the build collapses several distinct failure causes into a single
  // false return. This captures WHICH one fired and the free heap AT the failure point
  // (before any cleanup/clear that recovers heap), so the [E2] overlay + SD log report an
  // actionable number instead of the post-reset heap. floor/htmlSize are set where relevant
  // (LowHeap), else 0. Populated through createSectionFile's outFailure out-param.
  struct BuildFailure {
  ...
  // Failure captured by the most recent build attempt (createSectionFile / startBuild /
  // buildSomeMore). Read it before section.reset() to drive the [E2] overlay on any path.
  const BuildFailure& lastBuildFailure() const { return buildFailure_; }
  ...
  bool createSectionFile(int fontId, float lineCompression, bool extraParagraphSpacing, uint8_t paragraphAlignment,
                         uint16_t viewportWidth, uint16_t viewportHeight, bool hyphenationEnabled, bool embeddedStyle,
                         uint8_t imageRendering, bool focusReadingEnabled,
                         const std::function<void()>& popupFn = nullptr, BuildFailure* outFailure = nullptr);
 ...
 private:
  // Failure state captured during a startBuild/buildSomeMore/commit run and copied out
  // through createSectionFile's outFailure param. buildHtmlSize_ carries the inflated HTML
  // size from startBuild so Parse/Lut failures in later phases can report it.
  BuildFailure buildFailure_;
  uint32_t buildHtmlSize_ = 0;
  void recordBuildFailure(BuildFailure::Reason reason, uint32_t floor, uint32_t htmlSize, uint8_t streamSub = 0);
};
