...
- **Full-screen message / error / hint text** → wrap with `renderer.wrappedText(font, text, maxWidth, maxLines)` then loop `drawCenteredText` per line. Never draw a full-sentence `tr(...)` with a single `drawCenteredText` — it runs off both edges (worse on the narrower X3). `FullScreenMessageActivity`/`ConfirmationActivity` wrap centrally; per-screen error/hint draws (OPDS, KOReaderAuth, KOReaderSync, FontDownload, SdFirmwareUpdate) wrap inline.
- **On-SD debug log** → `SdDebugLog` (lib/SdDebugLog) appends to `/opds_debug.txt` (browsable on-device), mirrors to the serial log on **X4 only** (X3 is USB-locked), tags every line `[X3]`/`[X4]`, and flushes per line (survives the silent-restart). Off by default; the OPDS/KOSync/Auth activities enable it while active. `captureNetSnapshot()` bundles heap+contiguous+RSSI for network traces.
...
- **HTTPS handshakes are heap-starved on this device**: a TLS session peaks ~50KB and craters free heap to ~10KB / ~2KB largest-contiguous even with the inflate window reclaimed, so HTTP requests are scheme-gated to skip the TLS-sized preflight (`heapOkForUrl` in `KOReaderSyncClient`, `minContiguousForUrl` in `OpdsBookBrowserActivity` — `http://` uses a small bar, `https://` the full TLS bar; the OPDS contiguous gate is `MIN_CONTIGUOUS_HEAP_FOR_TLS`, lowered 24→18KB after X4 traces showed the 2nd fetch false-rejected by 12 bytes). In-tree mitigations (all from-source via `platformio.ini` `custom_sdkconfig`, which forces an ESP-IDF source build): `MBEDTLS_DYNAMIC_BUFFER` + `DYNAMIC_FREE_CONFIG_DATA`/`CA_CERT` (frees the CA bundle + key after the handshake — re-attached per connection by `crt_bundle_attach`); and `NoWifiSleep` (`WIFI_PS_NONE` for the duration of a request, in both `HttpDownloader` and `KOReaderSyncClient`) — modem-sleep was dropping handshake packets on the X3 and stalling the handshake to a 16.5s timeout.
- **RESOLVED VERDICT (June 2026 on-device): X4 does HTTPS fine; X3 HTTPS is unreliable — use HTTP on X3.** X4 KOSync (incl. PUT) and small/paged OPDS work; large OPDS feeds need small server-side pages (X4 large reads hit min-free ~2.3KB). X3 fails **back-to-back** TLS handshakes (1st often OK ~2-4s, 2nd/3rd time out). This is **NOT RAM** (X3/X4 both single-buffer `frameBuffer0[52272]`, near-identical ~62KB free) and **NOT firmware** (the network/TLS path has **zero device branches** — verified by grep — and runs blocking on the main task, so no I2C/display contention during the handshake). The only differences are board-level (X3 = UC81xx panel + BQ27220 I2C gauge; X4 = SSD1677 + ADC battery → different PCB/power/antenna) or test-environment. Tried and **REVERTED** as dead-ends: keep-alive single connection (holds the ~50KB arena open → no room for the 2nd request), WiFi `STATIC_RX_BUFFER_NUM` 8→12 (sped the 1st handshake only; 2nd + body reads still failed; taxes X4 ~6.4KB it doesn't need). Since the limit is not heap, freeing heap can't fix it.
- **`-fno-exceptions` + heap edge = every container growth is a latent `abort()`**: not just bare `new` — `std::vector::push_back`/`insert`, `std::string` append, and map inserts all call `operator new`, which aborts (does not return null) on OOM. Count-based caps don't protect against it; the failure is the *contiguous reallocation*. On the parse path (CSS rules, OPDS entries) guard growth with a `largest_free_block` check proportional to the next doubling and degrade gracefully (truncate / partial), never let the realloc abort. See SUMMARY §62/§64.
