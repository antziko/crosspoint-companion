...
- **HTTPS handshakes are heap-starved on this device**: a TLS session peaks ~55-63KB and craters free heap to single-KB even with the window reclaimed, so HTTP requests are scheme-gated to skip the TLS-sized preflight (`heapOkForUrl` in `KOReaderSyncClient`, `minContiguousForUrl` in `OpdsBookBrowserActivity` — `http://` uses a small bar, `https://` the full TLS bar). The window reclaim only helps a heap-**amount** shortage, not handshake fragmentation or socket exhaustion (SUMMARY §63 lesson). **OPDS** releases the window for the fetch on **both X3 and X4** (gate removed, SUMMARY §65) — X3's failure was the entry-vector OOM, which +32KB cures; X4 `http://` OPDS gets the same headroom cleanly. **X4 HTTPS OPDS may still regress** (the freed mid-session block fragments rather than helps the TLS handshake → 30s reads / "memory error"; revert to a `gpio.deviceIsX3()` gate if so), as does KOSync `BOOKMARKS_PUT` (3rd back-to-back handshake starves the socket, SUMMARY §47). Both need a bigger lever (framebuffer→heap, or a from-source `MBEDTLS_DYNAMIC_BUFFER` build) — deferred as out-of-scope vs. risk.
